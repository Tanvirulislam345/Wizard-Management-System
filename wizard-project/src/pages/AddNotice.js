...
import { ButtonMake, LayoutContiner, TextFieldMake } from "../styles/MetarialStyles";
...
const AddNotice = () => {
    ...
    useEffect(() => {
        ...
        axios.get('http://localhost:9000/employee')
            .then(res => {
                const data = res.data;
                const value1 = data.map(da => { return da.Email });
                setEmail1(value1)
            });

        axios.get('http://localhost:9000/client')
            .then(res => {
                const data = res.data;
                const value1 = data.map(da => { return da.Email });
                setEmail2(value1)
    ...
    const handleSubmit = () => {
        ...
        axios.post('http://localhost:9000/addnotice', newData)
            .then(res => {
                console.log(res)
            })
